...
- **인사이트 기반 동적 시각화**: 위 표준 차트 외에, 각 Stage/iteration에서 발견한 핵심 인사이트를 가장 잘 전달하는 시각화를 자율적으로 선택하여 추가한다. 고정 템플릿에 한정하지 않고, 해당 시점의 데이터 패턴과 분석 결과에 최적화된 차트를 만든다. 예시:
  - 특정 서브그룹에서 오분류가 집중되면 → 해당 그룹 상세 분포 차트
  - 구간화 전후 성능 차이가 있으면 → 구간별 타겟 비율 비교
  - 과적합이 의심되면 → Train vs CV 스코어 갭 시각화
  - 피처 간 상호작용이 발견되면 → 2D 히트맵 또는 조건부 분포
