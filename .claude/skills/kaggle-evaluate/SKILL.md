...
`$ARGUMENTS`에 `autopilot`이 포함되어 있으면 **자율 모드**로 동작한다. 자율 모드에서는 수렴 판단을 자동으로 수행하고 다음 단계로 진행한다. 그 외에는 **대화 모드**로 동작하며, 사용자에게 판단을 보고한다.
...
3. 각 그룹의 프로필을 분석한다:
   - 주요 피처별 분포 (상위 중요도 피처 중심)
   - 피처 조합별 오분류율 (상위 2개 범주형 피처 교차 등)
   - 오분류 집중 구간 식별
4. 주요 수치형 피처의 구간별 오분류율을 산출한다
...
1. **새 피처 생성**: 오분류 집중 구간의 패턴을 포착하는 피처
   - 피처 상호작용이 필요한 구간 → 상호작용 피처
   - 특정 서브그룹에서 집중 오분류 → 해당 그룹 타겟 피처
...
4. 대화 모드: 사용자에게 판단을 보고한다. 자율 모드: 자동으로 다음 단계를 결정한다
